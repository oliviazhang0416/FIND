...
#'          (This can be achieved by setting \code{extrasafe} == TRUE). If the isotonically-transformed
  ...
  for (j in 1:nscene){
    ...
    for (trial in 1:ntrial) {
      ...
      if (earlystop == 1) {
        ...
        MTD[trial] = select_mtd(method = "BOIN",
                                pT = pT,
                                EI = EI,
                                n_obs = n,
                                y_obs = y,
